package com.fh.baselib.widget.datepicker;
...
public class PickerView extends View {
    ...
    private void initPaint() {
        ...
        mPaintLine.setColor(ContextCompat.getColor(mContext, R.color.data_my_line));
        mPaintLine.setTextAlign(Align.CENTER);
...
//        mDarkColor = ContextCompat.getColor(mContext, R.color.date_picker_text_dark);
        mLightColor = ContextCompat.getColor(mContext, R.color.data_my_text);
        mDarkColor = ContextCompat.getColor(mContext, R.color.data_my_text);
    }
...
//        mTextSizeRange = maxTextSize - mMinTextSize;
        mMinTextSize = maxTextSize*0.5f;
        mTextSizeRange = maxTextSize*0.5f;
//        mTextSpacing = mMinTextSize * 2.8f;
        mTextSpacing = mMinTextSize * 4.8f;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        canvas.drawLine(0,mHeight/3,mWidth,mHeight/3 ,mPaintLine);
        canvas.drawLine(0,mHeight/3*2,mWidth,mHeight/3*2 ,mPaintLine);

        // 绘制选中的 text
        if (mDataList.get(mSelectedIndex).length() == 4 && !mDataList.get(mSelectedIndex).contains("年")) {
            drawText(canvas, mLightColor, mScrollDistance, mDataList.get(mSelectedIndex) + "年");
        } else  {
            drawText(canvas, mLightColor, mScrollDistance, mDataList.get(mSelectedIndex) + "月");
        }
        ...
        for (int i = 1; i <= mSelectedIndex; i++) {
            ...
            if (mDataList.get(mSelectedIndex - i).length() == 4 && !mDataList.get(mSelectedIndex - i).contains("年")) {
                drawText(canvas, mDarkColor, mScrollDistance - i * mTextSpacing,
                        mDataList.get(mSelectedIndex - i) + "年");
            } else  {
                drawText(canvas, mDarkColor, mScrollDistance - i * mTextSpacing,
                        mDataList.get(mSelectedIndex - i) + "月");
            }
        }
